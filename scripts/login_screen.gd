...
# ============================================================
# LOGIN SCREEN
# Waits for network config, connects, then handles login.
# Works correctly in both dev mode (instant) and release (async).
# ============================================================
...
func _ready() -> void:
	...
	# Config may have already loaded before this scene existed (dev mode).
	# Use call_deferred so the scene tree is fully ready before we act.
	call_deferred("_start")

func _start() -> void:
	if Network.config_loaded:
		# Already ready — connect immediately
		_launch_client()
	else:
		# Wait for async remote fetch to finish
		status_label.text = "Fetching server info..."
		Network.config_ready.connect(_launch_client, CONNECT_ONE_SHOT)
...
func _on_connected() -> void:
	status_label.text        = "Connected. Enter a username."
