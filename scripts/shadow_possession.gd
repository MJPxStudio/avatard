...
func tick(delta: float) -> void:
	...
	if _drain_timer >= 1.0:
		_drain_timer -= 1.0
		# We don't have a direct player ref in tick — drain happens
		# via _process hook in the player script watching _active state.
		# See: player checks hotbar slots for active shadow ability each frame.
		pass
